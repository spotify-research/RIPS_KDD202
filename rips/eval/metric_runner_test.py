...
def test_ListwiseMetricRunner():
    synthetic_logs = [
        (ListwiseLog('q1', ['d1', 'd2', 'd3'], [1, 0, 1]),
         {'test': {'d1': 0.9, 'd2': 0.1, 'd3': 0.4}})
    ]

    estimators = [PrecisionEstimator(cut_offs=[1, 2, 3, 10]),
                  RBPEstimator(cut_offs=[1, 2, 3, 10])]
    target_policies = [
        ('test', DeterministicLoggingPolicy())
    ]
    ...
    assert (list(estimates['test'][estimators[0].name]['est_mean']) ==
            pytest.approx([1.0, 1.0, 0.66666666, 0.2]))

    assert (list(estimates['test'][estimators[1].name]['est_mean']) ==
            pytest.approx([0.5, 0.75, 0.75, 0.75]))
...
def test_BeamListwiseMetricRunner():
    ...
    cutoffs = [1, 2, 3, 10]
    estimators = [
        PrecisionEstimator(cut_offs=cutoffs),
        RBPEstimator(cut_offs=[1, 2, 3, 10])
    ]
    ...
    (p
     ...
     | BeamListwiseMetricRunner(
                init_estimators_fn, init_target_policy_fn, max(cutoffs))
     | beam.Map(check)
     )
